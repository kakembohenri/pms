...
class Master extends DBConnection
{
	...
	function transfer_inmate()
	{
		...
		$query = $this->conn->query("SELECT * FROM transfers WHERE new_code='$new_code'");
		...
		// Create new inmate in transfers table

		$sql = "INSERT INTO transfers (inmate_id, cell_id, date_transfered, new_code) VALUES ('$inmate_id', '$cell_id', '$date_transfered', '$new_code')";

		$save = $this->conn->query($sql);
		if ($save) {
			$iid = !empty($inmate_id) ? $inmate_id : $this->conn->insert_id;
			$resp['iid'] = $iid;
			$resp['status'] = 'success';
			if (empty($inmate_id))
				$resp['msg'] = "Inmate transfer successfully saved.";
			else
				$resp['msg'] = " Inmate successfully updated.";
		} else {
			$resp['status'] = 'failed';
		...
		// 	}
		// }
		// $check = $this->conn->query("SELECT * FROM `transfers` where `new_code` = '{$this->conn->real_escape_string($new_code)}' " . (!empty($inmate_id) ? " and inmate_id != {$inmate_id} " : "") . " ")->num_rows;
		// if ($this->capture_err())
		// 	return $this->capture_err();
		// if ($check > 0) {
		// 	$resp['status'] = 'failed';
		...
		// 	exit;
		// }
		// $sql = "INSERT INTO `transfers` set {$data} ";
		// // if (empty($id)) {
		// // } else {
		// // 	$sql = "UPDATE `transfers` set {$data} where inmate_id = '{$id}' ";
		// // }
		// $save = $this->conn->query($sql);
		// if ($save) {
		// 	$iid = !empty($inmate_id) ? $id : $this->conn->insert_id;
		// 	$resp['iid'] = $iid;
		// 	$resp['status'] = 'success';
		// 	if (empty($inmate_id))
		// 		$resp['msg'] = "Inmate transfer successfully saved.";
...
switch ($action) {
	...
	case 'save_inmate':
		...
		break;
	case 'transfer_inmate':
		echo $Master->transfer_inmate();
		break;
	case 'delete_inmate':
